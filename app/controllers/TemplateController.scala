...
@Singleton
class TemplateController @Inject()(someServiceConnector: SomeServiceConnector,
  ...
  val submitMessage: Action[AnyContent] = Action.async { implicit request =>
    messageInputForm.bindFromRequest().fold(
      formWithErrors => {
        ...
        someServiceConnector.sendMessage(userMessage.message).map { response =>
          Redirect(routes.TemplateController.getHomepage()).addingToSession("MESSAGE" -> response)
        }
    )
  }
}